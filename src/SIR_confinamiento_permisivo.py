# SIR_confinamiento.py
# Autores: José Carlos Riego y Pablo Rodríguez Soria
# Modelo SIR discreto de red 2D que incluye el efecto de un confinamiento.
...
conf_perm_dir = os.path.join(data_dir, 'confinamiento_permisivo')
os.makedirs(conf_perm_dir, exist_ok=True)
os.makedirs(videos_dir, exist_ok=True)


# Parámetros
N = 2500  # Asegúrate de que N sea un cuadrado perfecto
...
dt = 5 # Incremento de tiempo en días => ESTE ES EL MEJOR dt POSIBLE
...
def update_network(frame_num, img, network, infection_time, ax, susceptible_count, infected_count, recovered_count):
    ...
    # Primer cuarto de pandemia: no hay aún confinamiento
    elif frame_num < steps // 4: 
        #print('Primer cuarto de pandemia: no hay aún confinamiento', frame_num)
    ...
    # A partir del primer cuarto de pandemia confinamos a la población: contactos por pares en la red
    elif frame_num >= steps // 4:
        #print('A partir del primer cuarto de pandemia confinamos a la población: contactos por pares en la red', frame_num)
        ...
        for i in range(side_length):
            for j in range(side_length):
                    ...
                    if new_network[i, j] == 1:  # Susceptible
                        ...
                        # Revisar el individuo de arriba en la red (si j es impar) o el de abajo (si j es impar)
                        # OJO: Aplicamos condiciones de contorno periódicas en la red 

                        # Confinamiento permisivo: dejamos que exista contacto entre vecinos de arriba y abajo
                        
                        if new_network[(i + 1) % side_length, j % side_length] == 2:
                            infected_neighbors += 1
...
# Ajustar los límites de los ejes para incluir el punto sqrt(N) en ambos ejes
...
ani = animation.FuncAnimation(fig, update_network, fargs=(img, network, infection_time, ax, susceptible_count, infected_count, recovered_count),
                              frames=steps, interval=200, repeat=False, blit = False)

# Guardar la animación como un vídeo => descomentar sólo si se va a guardar el vídeo porque da error el resto
#ani.save(os.path.join(videos_dir, f'simulacion_conf_perm_b_{b}_k_{k:.2f}_N_{N}_dt_{dt:.2f}_neigh_{neighbors}.mp4'), writer='ffmpeg', fps=1)  # Guardar como video MP4
...
#time_vector = np.linspace(0, dt*steps , steps)
time_vector = np.arange(0,tmax+1,dt)
#print(time_vector)
#print(len(infected_count))
...
# Título y ejes
title = f"Modelo SIR discreto (N = {N}, b = {b}, 1/k = {1/k:.2f}, dt = {dt:.2f})"
plt.title(title)
...
plt.vlines( time_vector[len(time_vector)//4 - 1], -0.5,1.5,colors = 'black', linestyles='--')
plt.fill_between(time_vector[len(time_vector)//4 - 1:], -0.5,1.5, color = 'gray', alpha = 0.3)
plt.text(time_vector.mean(), 0.8, 'Confinamiento permisivo', fontsize='xx-large') 
plt.ylabel('Proporción de la población')
...
base_name = f'conf_perm_b_{b}_k_{k:.2f}_N_{N}_dt_{dt:.2f}_neigh_{neighbors}'
np.savetxt(os.path.join(conf_perm_dir, f'susceptible_{base_name}.txt'), susceptible_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_perm_dir, f'infected_{base_name}.txt'), infected_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_perm_dir, f'recovered_{base_name}.txt'), recovered_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_perm_dir, f'infected_neighbors_count_{base_name}.txt'), infected_neighbors_count, fmt='%f', delimiter=',')
...
title = f"Modelo SIR determinista vs discreto con confinamiento (N = {N}, b = {b}, 1/k = {1/k:.2f}, dt = {dt:.2f}, Número de vecinos = {neighbors})"
plt.title(title)

# Si queremos graficar solamente los puntos
plt.scatter(time_vector, susceptible_count, label='Susceptibles', color='blue', alpha=0.6)
plt.scatter(time_vector, infected_count, label='Infectados', color='red', alpha=0.6)
plt.scatter(time_vector, recovered_count, label='Recuperados', color='green', alpha=0.6)
interval = 10  # Intervalo de 20 días para los ticks en el eje x
plt.xticks(ticks=np.arange(0, tmax + interval, interval))  # Ajustar para que se muestren los ticks cada 10 días
plt.vlines( time_vector[len(time_vector)//4 - 1], -0.5,1.5,colors = 'black', linestyles='--')
plt.fill_between(time_vector[len(time_vector)//4 - 1:], -0.5,1.5, color = 'gray', alpha = 0.3)
