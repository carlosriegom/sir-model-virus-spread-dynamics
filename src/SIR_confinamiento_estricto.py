...
conf_est_dir = os.path.join(data_dir, 'confinamiento_estricto')
os.makedirs(conf_est_dir, exist_ok=True)
        ...
        for i in range(side_length):
            for j in range(side_length):
                    ...
                    if new_network[i, j] == 1:  # Susceptible
                        ...
                        # Si i es par => contacta con el individuo de debajo
                        if i % 2 == 0:
                            if new_network[(i + 1) % side_length, j % side_length] == 2:
                                infected_neighbors += 1

                        # Si i es impar => contacta con el individuo de arriba
                        else:
                            if new_network[(i - 1) % side_length, j % side_length] == 2:
                                infected_neighbors += 1
...
#ani.save(os.path.join(videos_dir, f'simulacion_conf_est_b_{b}_k_{k:.2f}_N_{N}_dt_{dt:.2f}_neigh_{neighbors}.mp4'), writer='ffmpeg', fps=1)  # Guardar como video MP4
...
plt.text(time_vector.mean(), 0.8, 'Confinamiento estricto', fontsize='xx-large')
plt.xlabel('Tiempo (días)')
plt.ylabel('Proporción de la población')
...
base_name = f'conf_est_b_{b}_k_{k:.2f}_N_{N}_dt_{dt:.2f}_neigh_{neighbors}'
np.savetxt(os.path.join(conf_est_dir, f'susceptible_{base_name}.txt'), susceptible_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_est_dir, f'infected_{base_name}.txt'), infected_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_est_dir, f'recovered_{base_name}.txt'), recovered_count, fmt='%f', delimiter=',')
np.savetxt(os.path.join(conf_est_dir, f'infected_neighbors_count_{base_name}.txt'), infected_neighbors_count, fmt='%f', delimiter=',')
...
plt.ylabel('Proporción de la población')
plt.xlim(0, tmax)  # Establecer el límite del eje x 
plt.ylim(0, 1)  # Establecer el límite del eje y 
plt.show()